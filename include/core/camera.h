...
struct Camera {
    ...
    // constructor
    Camera() : eye(T(0), T(0), T(0)), viewdir(T(0), T(0), T(-1)), updir(T(0), T(1), T(0)), vfov(T(90)), width(800), height(600) { // default
    ...
    Camera(Point3<T> eye, Vec3<T> viewdir, Vec3<T> updir, T vfov, int width, int height) 
        : eye(eye), viewdir(viewdir), updir(updir), vfov(vfov), width(width), height(height) 
    {
